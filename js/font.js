...
var Char = function(container, char) {
    ...
    // 반대
    this.getAttr = function(dist, min, max) {
        var wght = min + (max * dist / maxDist);
        return Math.min(max, wght);
    }
    
    this.getAttr = function(dist, min, max) {
        var wdth = min + (max * dist / maxDist);
        return Math.min(max, wdth);
    }
...
var VFont = function() {
    ...
    this.setSize = function() {
        var fontSize = (window.innerWidth / (str.length / 2)) * 0.5; // 원하는 대로 조절하세요 (0.8은 예시)
